...
## 4. Google Calendar (optional — enables real availability + booking sync)
...
3. **APIs & Services → Credentials → Create OAuth client ID** (type: Web
   ...
   `{BACKEND_PUBLIC_URL}/integrations/calendar/callback`
4. Set `GOOGLE_CALENDAR_CLIENT_ID` / `GOOGLE_CALENDAR_CLIENT_SECRET` on the
   **backend**.
...
5. Deploy. Then go back through §3–5 and §7 above and register the
