...
export default function SignupPage() {
    ...
    const validateForm = () => {
        if (!formData.ownerName.trim()) {
            setLocalError(dict.validation.nameRequired);
            return false;
        }
        if (authMethod === 'email') {
            if (!formData.email.trim()) {
                setLocalError(dict.validation.emailRequired);
                return false;
            }
            if (formData.password.length < 6) {
                setLocalError(dict.validation.passwordMinLength);
                return false;
            }
        }
        if (!agreed) {
            setLocalError(dict.validation.agreeRequired);
            return false;
        }
        return true;
    };
    ...
    const handleSendOTP = async () => {
        if (!formData.phone) {
            setLocalError(dict.validation.phoneRequired);
            return;
        }
        if (!agreed) {
            setLocalError(dict.validation.agreeRequired);
    ...
    const getPasswordStrength = () => {
        ...
        if (password.length < 6) return { strength: 1, label: dict.signup.passwordWeak, color: 'bg-red-500' };
        if (password.length < 10) return { strength: 2, label: dict.signup.passwordFair, color: 'bg-yellow-500' };
        if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(password)) return { strength: 3, label: dict.signup.passwordStrong, color: 'bg-green-500' };
        return { strength: 2, label: dict.signup.passwordFair, color: 'bg-yellow-500' };
    };
    ...
    return (
        <div className="w-full max-w-md mx-auto">
            ...
            <div className="text-center mb-8">
                ...
                <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">{dict.signup.title}</h1>
                <p className="text-neutral-400">{dict.signup.subtitle}</p>
            </div>
            ...
            <div className="bg-neutral-900/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 p-8">
                ...
                <div className="space-y-4 mb-5">
                    <div>
                        <label className="block text-sm font-medium text-neutral-300 mb-1.5">{dict.signup.nameLabel}</label>
                        <div className="relative">
                            <User className="absolute start-3 top-1/2 -translate-y-1/2 w-5 h-5 text-neutral-500" />
                            <input
                                type="text"
                ...
                {authMethod === 'email' && (
                    <form onSubmit={handleEmailSignup} className="space-y-5">
                        <div>
                            <label className="block text-sm font-medium text-neutral-300 mb-1.5">{dict.login.emailLabel}</label>
                            <div className="relative">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-neutral-300 mb-1.5">{dict.login.passwordLabel}</label>
                            <div className="relative">
                ...
                {authMethod === 'phone' && !otpSent && (
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-neutral-300 mb-1.5">{dict.login.phoneLabel}</label>
                            <div className="relative">
                                <Phone className="absolute start-3 top-1/2 -translate-y-1/2 w-5 h-5 text-neutral-500" />
                                <input
                                    type="tel"
                                    ...
                                    dir="ltr"
                                    className="w-full ps-10 pe-4 py-3 bg-neutral-950 border border-neutral-800 rounded-xl text-white placeholder:text-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all text-start"
                                />
                            </div>
                            <p className="mt-2 text-xs text-neutral-500">{dict.login.phoneHint}</p>
                        </div>
                    ...
                    <form onSubmit={handleVerifyOTP} className="space-y-6">
                        <div className="text-center">
                            <p className="text-neutral-400 text-sm mb-1">
                                {dict.login.otpSentTo} <span dir="ltr" className="text-white font-medium">{formData.phone}</span>
                            </p>
                            <button
                                type="button"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-neutral-300 mb-2 text-center">{dict.login.otpLabel}</label>
                            <input
                                type="text"
                                value={otp}
                ...
                <div className="flex items-center gap-4 my-8">
                    ...
                    <span className="text-sm text-neutral-500 px-2">{dict.login.orContinueWith}</span>
                    <div className="flex-1 h-px bg-white/10" />
                </div>
                ...
                <p className="text-center text-neutral-500 mt-8">
                    ...
                    <Link href={href('/login')} className="text-blue-400 hover:text-blue-300 hover:underline font-semibold">
                        {dict.signup.signIn}
                    </Link>
                </p>
            </div>
