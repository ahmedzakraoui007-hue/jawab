/**
 * Booking actions the AI can actually invoke via Gemini function-calling.
 * This is what makes "Jawab checks availability and books real
 * appointments" true rather than just something the model narrates —
 * previously nothing wired the conversation to any real Calendar/Firestore
 * write, so the AI could confidently confirm a booking that never existed.
 */

import { adminDb, isAdminConfigured } from '@/lib/firebase-admin';
import { getAvailableSlots, createBookingEvent, formatSlotTime } from '@/lib/google-calendar';
...
export async function createBookingTool(
    ...
    if (!isAdminConfigured) {
        return { error: 'Booking system is not available right now — offer to have a team member confirm manually.' };
    }
...
export async function executeBookingFunction(
    ...
    if (name === 'check_availability') {
        return checkAvailabilityTool(args, ctx);
    }
    if (name === 'create_booking') {
        return createBookingTool(args, ctx);
    }
